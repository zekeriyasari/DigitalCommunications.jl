...
\begin{document}
    \begin{tikzpicture}[node distance=2.5cm, >=latex]
        ...
        \draw[->] (modulator) -- node[midway, anchor=south]{$s_{il}$} (channel);
        \draw[->] (channel) -- node[midway, anchor=south]{$r_{il}$} (detector);
        \draw[<-] (modulator.west) -- ++(-1cm, 0cm) node[anchor=south]{$m_i$};
        \draw[<-] (channel.south) -- ++(0cm, -1cm) node[anchor=north]{$n_{il}
        $};
